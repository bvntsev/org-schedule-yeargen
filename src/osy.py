...
def main():
    ...
    logging.basicConfig(filename="osy.log", level=(
        logging.DEBUG if len(sys.argv) > 1 and sys.argv[1] == "DEBUG=y"
        else logging.INFO), format=LOG_FORMAT)
    logger.info("Started")
...
#         not (datetime.MINYEAR <= user_year_int <= datetime.MAXYEAR):
...
#
#     first_study_day = datetime.datetime(user_year_int, 2, 2)
#
#     print("====MK_MONTHS====")
...
#             (str(iter_month) if iter_month > 9 else '0' + str(iter_month)) + \
#             '_' + curr_month + '/'
#
...
#         num_created_files = 0
#         for \
#         iter_day in range(1, calendar.monthrange(user_year_int, iter_month)[1] + 1):
#
#             curr_day = datetime.datetime(user_year_int, iter_month, iter_day)
#
...
#
#             org_schedule = day_content.get_day_content(curr_day, num_week)
